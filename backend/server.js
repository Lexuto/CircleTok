...
async function initDB() {
    ...
    await db.exec(`
        CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, telegram_id TEXT UNIQUE, username TEXT, first_name TEXT);
        CREATE TABLE IF NOT EXISTS videos (id INTEGER PRIMARY KEY AUTOINCREMENT, file_id TEXT, user_telegram_id TEXT, status TEXT DEFAULT 'pending', likes_count INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);
    `);
...
app.post('/api/upload', upload.single('video'), async (req, res) => {
    try {
        ...
        const message = await bot.telegram.sendVideoNote('@CircleTokpending', { source: req.file.buffer }, { duration: 60, length: 640 });
        ...
        await bot.telegram.sendMessage(process.env.MODERATOR_ID, `📹 Новое видео от @${user?.username || 'user'}\nID: ${videoId}`);
...
app.get('/api/feed', async (req, res) => {
    try {
        ...
        const videos = await db.all(`
            ...
            WHERE v.status IN ('approved', ?) ORDER BY v.created_at DESC LIMIT 30
